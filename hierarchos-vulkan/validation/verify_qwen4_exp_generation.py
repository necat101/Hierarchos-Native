#!/usr/bin/env python3
"""Strict per-step Qwen4-Exp generation parity against the local Transformers checkout."""
...
from verify_hf_logits import QWEN4_EXP_STRICT_FAMILIES, REPO, build_binary, save_reference_checkpoint, tiny_models
    ...
    with torch.no_grad():
        for _ in range(steps):
            ...
            if use_cache and all(layer_type == "linear_attention" for layer_type in model.config.layer_types):
                # Synthetic all-linear configs have no sequence-bearing QSA KV
                # cache. Both effective masks are None for this unpadded case.
...
def run_family(binary, root, name, model, steps, device_index):
    ...
    # Length five deliberately leaves a trailing QSA token for compress_ratio=2.
    # Successive decode steps cross complete-block boundaries while PLE keeps its
    # n-gram history and dilation-3 short-convolution state.
    input_ids = torch.tensor([[1, 5, 9, 3, 7]], dtype=torch.long)
...
def main() -> None:
    ...
    parser.add_argument("--families", nargs="+", choices=QWEN4_EXP_STRICT_FAMILIES)
    args = parser.parse_args()
    if args.steps <= 0:
        parser.error("--steps must be positive")

    requested = set(args.families or QWEN4_EXP_STRICT_FAMILIES)
    ...
    if missing:
        raise RuntimeError(f"missing Qwen4-Exp fixtures: {sorted(missing)}")

    binary = build_binary("transformer_generation_parity")
    with tempfile.TemporaryDirectory(prefix="vulkan-qwen4-exp-generation-") as directory:
