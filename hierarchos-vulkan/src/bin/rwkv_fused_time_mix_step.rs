...
fn main() -> Result<()> {
    ...
    let model_dir = model_dir.context("fused RWKV runner requires --model-dir")?;
    let prefix = prefix.context("fused RWKV runner requires --prefix")?;
    let case: Case = serde_json::from_slice(&fs::read(case_path)?)?;

    if case.width == 0 || case.width % case.head_size != 0 {
        anyhow::bail!(
    ...
    let mut op = RwkvTimeMixCoreOp::from_model_package_fused(
        device,
        model_dir,
        &prefix,
        case.head_size,
    ...
    let result = op.forward_backward_fused(
        case.batch,
        &case.state,
        &case.x_norm,
        &case.previous,
